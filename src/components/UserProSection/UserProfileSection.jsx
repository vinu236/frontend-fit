...
import { setPlanActive } from "../../Redux/planActiveSlice";
const UserProfileSection = ({ src, onClick }) => {
  ...
  const bmi=useSelector(store=>store.bmi.bmi)
  ...
  const getUser = async () => {
    ...
    try {
      ...
      if (status === 200) {
        ...
        dispatch(setBmi(data?.getUser.Bmi))
        dispatch(setPlanActive(data?.getUser))
      }
    } catch (error) {
      console.log(error)
      if(error.response){
  ...
  console.log(user);
  // Object.keys(user).length ==! 0 ? (<PageLoader/>):
  return (
    <div className="text-red-200 min-h-[86vh] bg-black  w-[254px] shadow-lg flex flex-col items-center gap-4  ">
      <label
      ...
      <div>
        <span className={`text-white tracking-wider ${bmi ? bmi.value:"text-red-600  text-md inline font-bold"}`}>Bmi:{bmi ? bmi.value:" Not Saved"} </span>
      </div>
      <div>
      <span className={`text-white tracking-wider ${bmi ? bmi.type:"text-red-600  text-md inline font-bold"}`}>Bmi:{bmi ? bmi.type:" Not Saved"} </span>
      </div>
      <Button
